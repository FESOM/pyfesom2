...
def plot(mesh, data, cmap=None, influence=80000, box=[-180,180,-89,90],
    ...
    if titles:
        ...
        if len(titles)!=len(data):
            raise ValueError("The number of titles do not match the number of data fields, please adjust titles (or put to None)")
    
    if ((rowscol[0]*rowscol[1]) < len(data)):
        raise ValueError("Number of rows*columns is smaller than number of data fields, please adjust rowcol.")
...
#     flf = Dataset(ifile)
    ...
    for datainstance in data:
        ...
        if interp =='nn':
            ofesom = fesom2regular(datainstance, mesh, lonreg2, latreg2,
                                      distances_path=distances_path, inds_path=inds_path,
                                      radius_of_influence=radius_of_influence, basepath=basepath)
            interpolated.append(ofesom)
        elif interp == 'idist':
            ofesom = fesom2regular(datainstance, mesh, lonreg2, latreg2, 
                                      distances_path=distances_path, inds_path=inds_path,
                                      radius_of_influence=radius_of_influence, how = 'idist', k = 5,
                                      basepath=basepath)
            interpolated.append(ofesom)
        elif interp == 'linear':
            ofesom = fesom2regular(datainstance, mesh, lonreg2, latreg2, 
                                      how = 'linear', qhull_path = qhull_path, basepath=basepath
                                     )
            interpolated.append(ofesom)
        elif interp == 'cubic':
            ofesom = fesom2regular(datainstance, mesh, lonreg2, latreg2, basepath=basepath,
                                      how = 'cubic',
                                     )
            interpolated.append(ofesom)
    ...
    m2 = np.where(((lonreg2 == -180.)&(latreg2>71.5)), True, mask)
...
#     m2 = np.where(((lonreg2 == 180.)&(latreg2<65.33)), True, m2)
    ...
    if mapproj == 'merc':
        fig, ax = plt.subplots(rowscol[0],rowscol[1], subplot_kw=dict(projection=ccrs.Mercator()),
                                                    constrained_layout=True,
                                                    figsize=figsize)
    elif mapproj == 'pc':
        fig, ax = plt.subplots(rowscol[0],rowscol[1], subplot_kw=dict(projection=ccrs.PlateCarree()),
                                                    constrained_layout=True,
                                                    figsize=figsize)
    elif mapproj == 'np':
        fig, ax = plt.subplots(rowscol[0],rowscol[1], subplot_kw=dict(projection=ccrs.NorthPolarStereo()),
                                                    constrained_layout=True,
                                                    figsize=figsize)
    elif mapproj == 'sp':
        fig, ax = plt.subplots(rowscol[0],rowscol[1], subplot_kw=dict(projection=ccrs.SouthPolarStereo()),
                                                    constrained_layout=True,
                                                    figsize=figsize)
    elif mapproj == 'rob':
        fig, ax = plt.subplots(rowscol[0],rowscol[1], subplot_kw=dict(projection=ccrs.Robinson()),
                                                    constrained_layout=True,
                                                    figsize=figsize)
    ...
    for ind, data_int in enumerate(interpolated):
        ...
        if ptype == 'cf':
            ...
            data_int_cyc, lon_cyc = add_cyclic_point(data_int, coord=lonreg)
            image = ax[ind].pcolormesh(lon_cyc,\
                         latreg,\
                         data_int_cyc,
                             vmin = mmin,
                             vmax = mmax,
...
def plot_transect(data3d, mesh, lon_start,
    ...
    if not isinstance(data3d, list):
        ...
        image = ax.contourf( dist, np.abs(mesh.zlev[:depth_index]), transect_data[:,:depth_index].T,
        ...
        for ind, data in enumerate(data3d):
            ...
            image = ax[ind].contourf( dist, np.abs(mesh.zlev[:depth_index]), transect_data[:,:depth_index].T,
                                levels = levels, cmap = cmap, extend='both')
            ax[ind].invert_yaxis()
