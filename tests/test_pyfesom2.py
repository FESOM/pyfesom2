...
def test_readmesh():
    ...
    assert os.path.exists(
        os.path.join(my_data_folder, "pi-grid", "pickle_mesh_py3_fesom2")
    )
    ...
    assert os.path.exists(
        os.path.join(my_data_folder, "pi-grid", "joblib_mesh_py3_fesom2")
    )
...
def test_get_data():
    ...
    # return dask array
    temp = get_data(
        data_path, "temp", [1948, 1949], mesh, depth=200, how="max", compute=False
    )
